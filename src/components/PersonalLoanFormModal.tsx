...
interface PersonalLoanFormModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const DocumentUploadItem: React.FC<{
    ...
    clientDocs: Document[];
    onUpload: (file: File, docId: string, password?: string) => void;
    onDelete: (docId: string) => void;
    onPreview: (doc: Document) => void;
}> = ({ docLabel, docId, docDesc, needsPassword, clientDocs, onUpload, onDelete, onPreview }) => {
    ...
    const document = clientDocs.find(d => d.fileName.startsWith(`${docId}_`));
    ...
    return (
        <div className="flex flex-wrap justify-between items-center">
            <div>
                <p className="font-semibold text-sm text-slate-800 dark:text-slate-200">{docLabel}</p>
...
export const PersonalLoanFormModal: React.FC<PersonalLoanFormModalProps> = ({ isOpen, onClose, onSave, client, documents, onUploadDocument, onDeleteDocument }) => {
  const [formData, setFormData] = useState({ loanPurpose: '', monthlyIncome: '' });
  const [coApplicants, setCoApplicants] = useState<CoApplicantDetails[]>([]);
  ...
  useEffect(() => {
    if (isOpen) {
      setFormData({
        loanPurpose: (client.generalInformation as any)?.loanPurpose || '',
        monthlyIncome: client.generalInformation?.monthlyIncome || '',
      });
      setCoApplicants(client.generalInformation?.coApplicants || []);
    }
  }, [isOpen, client]);

  const handleDocUpload = useCallback((file: File, docId: string, password?: string) => {
    const prefixedFileName = `${docId}_${file.name}`;
    onUploadDocument({
        clientId: client.id,
  ...
  const removeCoApplicant = (id: string) => {
    setCoApplicants(coApplicants.filter((coApp) => coApp.id !== id));
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-start pt-16 z-50 overflow-y-auto">
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-2xl transform transition-all mb-8">
        <div className="px-6 py-4 flex justify-between items-center border-b border-slate-200 dark:border-slate-700">
          <h2 className="text-xl font-semibold text-slate-800 dark:text-slate-100">Personal Loan Application Form</h2>
        ...
        <form onSubmit={handleSubmit}>
          <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
            ...
            <AccordionSection title="1. Loan Details" defaultOpen>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label htmlFor="loanPurpose" className="block text-sm font-medium text-slate-700 dark:text-slate-300">Purpose of Loan</label>
                  <input type="text" name="loanPurpose" value={formData.loanPurpose} onChange={handleChange} required className={inputStyles} />
                </div>
                <div>
                  <label htmlFor="monthlyIncome" className="block text-sm font-medium text-slate-700 dark:text-slate-300">Gross Monthly Income (₹)</label>
                  <input type="number" name="monthlyIncome" value={formData.monthlyIncome} onChange={handleChange} required className={inputStyles} />
                </div>
              </div>
            </AccordionSection>
                  ...
                  {coApplicants.map((coApp, index) => (
                    <div key={coApp.id} className="p-4 bg-slate-50 dark:bg-slate-900/50 rounded-md border border-slate-200 dark:border-slate-700 relative">
                      <div className="flex justify-between items-center border-b border-slate-300 dark:border-slate-600 pb-2 mb-4">
                        ...
                        <button type="button" onClick={() => removeCoApplicant(coApp.id)} className="text-red-500 hover:text-red-700 dark:hover:text-red-400" title="Remove Co-applicant"><TrashIcon className="w-5 h-5"/></button>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                       ...
                       <div className="mt-4">
                          ...
                          {applicantKycDocumentList.map(docItem => (
                            <div key={`coapp_${index}_${docItem.name}`} className="py-3 border-b border-slate-200 dark:border-slate-700 last:border-b-0">
                              <DocumentUploadItem 
                                docLabel={`Co-applicant ${index + 1} ${docItem.name}`}
                                docId={`co-applicant-${coApp.id}-${docItem.name.replace(/\s+/g, '-')}`}
