...
export const Dashboard: React.FC<DashboardProps> = ({ clients, tasks, users, onSelectClient, currentUser, notifications, onMarkAllAsRead, onToggleSidebar, navigateToFilteredView, onOpenDialPad }) => {
    ...
    const { activeUserCounts, totalActiveUsers } = useMemo(() => {
        const counts = users.filter(u => u.status === 'Active').reduce((acc: Record<string, number>, user: User) => {
